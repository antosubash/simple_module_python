...
export async function resolvePage(name: string): Promise<React.ComponentType<any>> {
    const loader = pages[name];
    if (!loader) {
        throw new Error(
            `Page "${name}" not found. Available pages: ${Object.keys(pages).join(', ')}`,
        );
    }
