...
class TestAuditLink:
    def test_url_for_substitutes_the_id(self):
        link = AuditLink(entity_type="users_user", url_template="/users/admin/{id}")
    ...
    def test_template_without_placeholder_is_rejected(self):
        ...
        with pytest.raises(ValueError, match="contains no"):
            AuditLink(entity_type="users_user", url_template="/users/admin")
...
class TestAuditLinkRegistry:
    def test_get_returns_none_for_unclaimed_tables(self):
        ...
        assert AuditLinkRegistry().get("permissions_user_permission") is None

    def test_register_and_get(self):
        reg = AuditLinkRegistry()
        link = AuditLink(entity_type="users_user", url_template="/users/admin/{id}", label="User")
        reg.register(link)
        assert reg.get("users_user") is link

    def test_conflicting_claims_raise(self):
        """Two modules mapping one table means one of them silently loses."""
        reg = AuditLinkRegistry()
        reg.register(AuditLink(entity_type="users_user", url_template="/a/{id}"))
        with pytest.raises(ValueError, match="Two modules claim"):
            reg.register(AuditLink(entity_type="users_user", url_template="/b/{id}"))
    ...
    def test_all_links_is_a_copy(self):
        ...
        reg.register(AuditLink(entity_type="users_user", url_template="/a/{id}"))
        reg.all_links.clear()
        assert reg.get("users_user") is not None
