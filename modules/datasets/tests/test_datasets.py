...
class TestDatasetsViews:
    """The CUD view endpoints exist so Inertia's ``router.post/patch/delete``
    calls receive a redirect (which Inertia follows) rather than raw JSON
    (which Inertia rejects with ``Inertia response required`` on the client).
    """

    async def _create_via_api(
        self, authenticated_client: httpx.AsyncClient, name: str = "Seed"
    ) -> int:
    ...
    async def test_upload_view_returns_303_redirect(self, authenticated_client: httpx.AsyncClient):
    ...
    async def test_upload_view_rejects_unknown_kind(self, authenticated_client: httpx.AsyncClient):
        files = {"file": ("a.geojson", b"{}", "application/json")}
        resp = await authenticated_client.post(
            "/datasets/",
            data={"name": "x", "kind": "not_a_kind"},
    ...
    async def test_update_view_returns_303_redirect(self, authenticated_client: httpx.AsyncClient):
        item_id = await self._create_via_api(authenticated_client, name="Before")
    ...
    async def test_update_view_missing_id_404(self, authenticated_client: httpx.AsyncClient):
        resp = await authenticated_client.patch(
            "/datasets/9999",
            json={"name": "ghost"},
            follow_redirects=False,
    ...
    async def test_delete_view_returns_303_and_removes_row(
        self, authenticated_client: httpx.AsyncClient
    ):
        item_id = await self._create_via_api(authenticated_client, name="Doomed-View")
    ...
    async def test_delete_view_missing_id_404(self, authenticated_client: httpx.AsyncClient):
