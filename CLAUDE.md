...
Standard mixins in `simple_module_db.mixins`: `AuditMixin`, `SoftDeleteMixin` (bypass the read filter with `stmt.execution_options(include_deleted=True)`; purge by deleting an already-trashed row, or with `hard_delete(session, obj)`), `MultiTenantMixin`, `VersionedMixin`. The soft-delete/tenant filters cover selects that name no ORM entity too — `select(func.count()).select_from(Model)`, counts over subqueries, and Core selects over `Model.__table__` (GH #332). The per-request session (`get_db`) auto-commits **only if** there are pending writes (via `after_flush` listener); otherwise rollback. Service code should **not** call `session.commit()` — flush if you need DB-assigned values. DML executed through the session (`session.execute(update(Model)...)`) counts as a write; a raw `text("UPDATE ...")` does not, and needs `mark_written(session)`. The commit fires in `CommitBeforeResponseMiddleware`, at the ASGI `http.response.start` message, so a client that creates a row and immediately reads it back in a second request sees it — FastAPI runs a `yield` dependency's exit code *after* the response is delivered, which used to make that a deterministic 404 (GH #257). `get_db` keeps the same commit in its own exit code as a fallback for when the middleware isn't in the stack; whichever runs first wins.
...
- **Configuration lives in the DB, not `.env`.** Only `SM_DATABASE_URL` is required; `SM_REDIS_URL` and the optional `SM_USERS_BOOTSTRAP_*` are the only others an operator normally sets. `SM_ENVIRONMENT`/`SM_DEBUG`/`SM_VITE_DEV_URL` describe the process rather than the configuration. Precedence is always **env → DB → default**; env must keep winning or existing deployments change behaviour silently on upgrade.
- **Settings are read twice per boot.** `create_app` builds the module list, the i18n registry and the middleware stack in Phases 1 and 8 — all before the lifespan opens the DB — so `_preapp_config.merge_host_settings` does one short-lived read *before* Phase 1. The lifespan then hydrates again for runtime edits. A `HostSettings` field consumed at build time is inert without that first read: this is why `i18n_*` and `multi_tenant` edits in the admin UI used to do nothing. **`host/main.py` must call `merge_host_settings()`, not `Settings()`** — it passes settings into `create_app`, so `create_app`'s own fallback never fires there.
- **`SM_SECRET_KEY` is optional**: unset generates a key and stores it (atomically — concurrent workers must converge, or they invalidate each other's sessions). An explicit env value wins and is never copied into the DB.
